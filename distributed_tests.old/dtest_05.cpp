...
int main(int argc, char **argv)
{
    ...
    int SIZE1 = 80000; // Columns
    int SIZE0 = 53000; // Rows
    ...
    tiramisu::constant ten("ten", tiramisu::expr(10), tiramisu::p_int32, true, NULL, 0, &dtest_05);
    tiramisu::constant twenty("twenty", tiramisu::expr(20), tiramisu::p_int32, true, NULL, 0, &dtest_05);
    ...
    send_recv fan_out = computation::create_transfer(
            "[Ny, Nx, one, twenty]->{fan_out_s[c,d,y,x]: 0<=c<one and one<=d<twenty and d*4000<=y<(d+1)*4000 and 0<=x<Nx+8}", // we want 10 nodes
            "[Ny, Nx, one]->{fan_out_r[y,x]: 0<=y<Ny+6-4000 and 0<=x<Nx+8}", 0, d, 0, var("c1"), async_nonblock, // TODO I have to specify the transformed iterator here. Not good.
    ...
    send_recv fan_in = computation::create_transfer(
            ...
            "[My, Mx, one, twenty]->{fan_in_r[c,d,y,x]: 0<=c<one and one<=d<twenty and d*4000<=y<(d+1)*4000 and 0<=x<Mx}", var("c1"), 0, var("c3"), 0, async_nonblock,
    ...
    fan_in_s->shift(y1, 1);
    tiramisu::wait wait_fan_in_s(fan_in_s->operator()(y, x), &dtest_05);
    wait_fan_in_s.split(y, 4000, y1, y2);
    wait_fan_in_s.shift(y1, 1);
    fan_out_s->interchange(d, y);
    tiramisu::wait wait_fan_out_s(fan_out_s->operator()(c, d, y, x), &dtest_05);
     ...
     */
    var y3("y3"), y4("y4");
    bx.get_update(0).split(y2, 100, y3, y4);
    bx.get_update(0).tag_parallel_level(y3);
    bx.get_update(0).set_loop_level_names({0}, {"y1"});
    bx.get_update(1).split(y2, 100, y3, y4);
    bx.get_update(1).tag_parallel_level(y3);
    bx.get_update(1).set_loop_level_names({0}, {"y1"});
    by.get_update(0).split(y2, 100, y3, y4);
    by.get_update(0).tag_parallel_level(y3);
    by.get_update(0).set_loop_level_names({0}, {"y1"});
    by.get_update(1).split(y2, 100, y3, y4);
    by.get_update(1).tag_parallel_level(y3);
    by.get_update(1).set_loop_level_names({0}, {"y1"});

    //    wait_fan_out_s.tag_parallel_level(d);
    ...
    wait_fan_out_s.before(*fan_in_r, y);
    ...
    wait_fan_in_s.tag_distribute_level(y1);

    /*
     * Set the message tags
     */

    static_cast<send*>(&fan_out_s->get_update(0))->override_msg_tag(var("c3")%4000);//var("c5")-var("c3")*4000);
    ...
    //    wait_fan_out_s.offset = -(var("c3") - 1) * 4000;
    fan_in_s->offset = -(var("c1") - 1) * 4000 * by_ext_0;
    //    wait_fan_in_s.offset = -(var("c1") - 1) * 4000;
    ...
    // TODO have to make this 1 rank bigger than necessary because I can't do the offset 
    // when writing to the wait buffer initially
    buffer fan_out_req_s_buff("fan_out_req_s_buff", {20*4000}, tiramisu::p_req_ptr, a_temporary, &dtest_05);
    static_cast<send*>(&fan_out_s->get_update(0))->set_req_access("{fan_out_s[c,z,y,x]->fan_out_req_s_buff[y-4000]}");
    buffer fan_in_req_s_buff("fan_in_req_s_buff", {4000}, tiramisu::p_req_ptr, a_temporary, &dtest_05);
        fan_in_s->set_req_access("{fan_in_s[y,x]->fan_in_req_s_buff[y%4000]}");
