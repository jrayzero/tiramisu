...
int main() {
    ...
    tiramisu::wait kernel_by_wait("[procs, rows_per_node]->{by_wait[q,y]: 0<=q<procs and 0<=y<rows_per_node/500}", by(y, x), kernel, true, &blur_dist);
    ...
    gpu_to_cpu.os->before(gpu_to_cpu_wait, y2);//computation::root);
