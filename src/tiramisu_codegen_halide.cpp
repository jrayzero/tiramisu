...
template <typename T>
tiramisu::expr get_tiramisu_expr_as_tiramisu_type(T val, primitive_t ttype) {
    switch (ttype) {
        case p_uint8: return tiramisu::expr((uint8_t)val);
        case p_uint16: return tiramisu::expr((uint16_t)val);
        case p_uint32: return tiramisu::expr((uint32_t)val);
        case p_uint64: return tiramisu::expr((uint64_t)val);
        case p_int8: return tiramisu::expr((int8_t)val);
        case p_int16: return tiramisu::expr((int16_t)val);
        case p_int32: return tiramisu::expr((int32_t)val);
        case p_int64: return tiramisu::expr((int64_t)val);
        case p_float32: return tiramisu::expr((float)val);
        case p_float64: return tiramisu::expr((double)val);
                                                   ...
                                                   bool return_buffer_accesses)
{
                                                    ...
                                                    (exp.get_op_type() == tiramisu::o_address_of)))
    {
        ...
        tiramisu::computation *access_op_comp = nullptr;
        if (exp.get_name() != "dummy") {
            access_op_comp = computations_vector[0];
        }
...
tiramisu::expr tiramisu_expr_from_isl_ast_expr(isl_ast_expr *isl_expr, bool convert_to_loop_type)
{
    ...
    if (isl_ast_expr_get_type(isl_expr) == isl_ast_expr_int)
    {
        ...
        if (!convert_to_loop_type) {
            ...
            result = get_tiramisu_expr_as_tiramisu_type(isl_val_get_num_si(init_val), global::get_loop_iterator_data_type());
        }
    ...
    else if (isl_ast_expr_get_type(isl_expr) == isl_ast_expr_id)
    {
        ...
        if (!convert_to_loop_type) {
            ...
            result = tiramisu::var(global::get_loop_iterator_data_type(), name_str);
        }
    }
    else if (isl_ast_expr_get_type(isl_expr) == isl_ast_expr_op)
    {
        ...
        switch (isl_ast_expr_get_op_type(isl_expr))
        {
            ...
            case isl_ast_op_pdiv_q:
                ...
                result = tiramisu::expr(tiramisu::o_cast, result.get_data_type(), result);
                break;
...
Halide::Expr halide_expr_from_isl_ast_expr(isl_ast_expr *isl_expr, bool convert_to_loop_type)
{
    Halide::Expr result;
    ...
    else if (isl_ast_expr_get_type(isl_expr) == isl_ast_expr_op)
    {
        ...
        switch (isl_ast_expr_get_op_type(isl_expr))
        {
            ...
            case isl_ast_op_pdiv_q:
                result = Halide::Internal::Div::make(op0, op1);
                result = Halide::Internal::Cast::make(result.type(), Halide::floor(result));
                break;
            case isl_ast_op_zdiv_r:
            case isl_ast_op_pdiv_r:
        ...
        bool is_a_child_block)
{
    ...
    else if (isl_ast_node_get_type(node) == isl_ast_node_for)
    {
        ...
        */
        ...
        while (tt < tagged_stmts.size())
        {
            if (tagged_stmts[tt] != "")
            {
                if (fct.should_map_to_gpu(tagged_stmts[tt])) {
                    std::pair<int, int> range = fct.gpu_ranges[tagged_stmts[tt]];
        ...
        if (handoff_gpu_loop && process_gpu_loop) {
                ...
                if (is_iter) {
                    Halide::Expr var =
                            Halide::Internal::Variable::make(
                                    halide_type_from_tiramisu_type(global::get_loop_iterator_data_type()), arg_name + "_" + kernel);
                    kernel_params.push_back(var);
                } else {
                    Halide::Expr var =
                            Halide::Internal::Variable::make(
                                    halide_type_from_tiramisu_type(global::get_loop_iterator_data_type()), arg_name);
                    kernel_params.push_back(var);
                }
            }
            ...
            if (comp->is_nonblock_or_async) { // needs an event buffer
                ...
                const auto &wait_buffer_entry = fct.get_buffers().find(
                        isl_map_get_tuple_name(comp->wait_access_map, isl_dim_out));
                assert(wait_buffer_entry != fct.get_buffers().end());
            ...
            for (auto n : std::get<3>(kernel_fn_and_buffer_names)) {
                ...
                std::string f = "_no_event";
                std::string repl = "";
                find_and_replace(let_name, f, repl);
                Halide::Expr let_expr = n.second;
                result = Halide::Internal::LetStmt::make(let_name, let_expr, result);
            }
    ...
    else if (isl_ast_node_get_type(node) == isl_ast_node_user)
    {
        ...
        else
        {
            ...
            isl_id_free(id);
            // see if computation is a GPU computation
    ...
    for (const auto &b : this->get_buffers())
    {
            ...
            buf->get_auto_allocate() == true)
        {
            ...
            for (int i = buf->get_dim_sizes().size() - 1; i >= 0; --i)
            {
                const auto sz = buf->get_dim_sizes()[i];
                std::vector<isl_ast_expr *> ie = {};
                halide_dim_sizes.push_back(generator::halide_expr_from_tiramisu_expr(this, ie, sz, nullptr));
            }
            ...
            Halide::Expr bytes((uint64_t)1);
            for (int i = buf->get_dim_sizes().size() - 1; i >= 0; --i)
            {
                const auto sz = buf->get_dim_sizes()[i];
                std::vector<isl_ast_expr *> ie = {};
                bytes *= Halide::cast(Halide::UInt(64), generator::halide_expr_from_tiramisu_expr(this, ie, sz, nullptr));
            }
 ...
 */
void tiramisu::computation::create_halide_assignment()
{
    DEBUG_FCT_NAME(3);
    DEBUG_INDENT(4);
    ...
    if (this->is_let_stmt())
    {
        ...
        if (!this->is_library_call() || this->lhs_argument_idx != -1) { // This has an LHS to compute.
            ...
            if (this->lhs_access_type == tiramisu::o_access) {
                ...
                // TODO(Jess) don't process these separately. That's a pain
                if (this->library_call_name == "tiramisu_cudad_memcpy_async_h2d" || this->library_call_name == "tiramisu_cudad_memcpy_async_d2h") {
                    ...
                    Halide::Expr call_stream = Halide::Internal::Load::make(Halide::Handle(), "streams",
                                                                            this->library_call_args[3].get_int32_value(), Halide::Buffer<>(),
                                                                            Halide::Internal::Parameter(), Halide::Internal::const_true());
            ...
            if (this->wait_argument_idx != -1) {
                ...
                result = Halide::Internal::Call::make(Halide::Handle(1, wait_type.handle_type),
                                                      Halide::Internal::Call::address_of, {result},
                                                      Halide::Internal::Call::Intrinsic);
...
Halide::Expr generator::halide_expr_from_tiramisu_expr(const tiramisu::function *fct, std::vector<isl_ast_expr *> &index_expr,
                                                       const tiramisu::expr &tiramisu_expr, tiramisu::computation *comp)
{
    ...
    if (tiramisu_expr.get_expr_type() == tiramisu::e_val)
    {
        ...
        {
            result = Halide::Expr(tiramisu_expr.get_string_value());
        }
    }
    else if (tiramisu_expr.get_expr_type() == tiramisu::e_op)
    {
        ...
        switch (tiramisu_expr.get_op_type())
        {
            ...
            case tiramisu::o_ne:
                ...
                break;
            case tiramisu::o_bitwise_and:
                result = Halide::Internal::Call::make(op0.type(), Halide::Internal::Call::bitwise_and, {op0, op1},
                                                      Halide::Internal::Call::PureIntrinsic);
                DEBUG(3, tiramisu::str_dump("op type: o_bitwise_and"));
                break;
            case tiramisu::o_bitwise_or:
                result = Halide::Internal::Call::make(op0.type(), Halide::Internal::Call::bitwise_or, {op0, op1},
                                                      Halide::Internal::Call::PureIntrinsic);
                DEBUG(3, tiramisu::str_dump("op type: o_bitwise_or"));
                break;
            case tiramisu::o_bitwise_xor:
                result = Halide::Internal::Call::make(op0.type(), Halide::Internal::Call::bitwise_xor, {op0, op1},
                                                      Halide::Internal::Call::PureIntrinsic);
                DEBUG(3, tiramisu::str_dump("op type: o_bitwise_xor"));
                break;
            case tiramisu::o_bitwise_not:
                result = Halide::Internal::Call::make(op0.type(), Halide::Internal::Call::bitwise_not, {op0},
                                                      Halide::Internal::Call::PureIntrinsic);
                DEBUG(3, tiramisu::str_dump("op type: o_bitwise_not"));
                break;
            case tiramisu::o_is_nan:
                result = Halide::is_nan(op0);
                DEBUG(3, tiramisu::str_dump("op type: o_is_nan"));
                break;
            case tiramisu::o_pow:
                result = Halide::pow(op0, op1);
                DEBUG(3, tiramisu::str_dump("op type: o_pow"));
                break;
            case tiramisu::o_type:
            {
...
void tiramisu::generator::_update_producer_expr_name(tiramisu::expr &current_exp, std::string name_to_replace,
    ...
    if ((current_exp.get_expr_type() == tiramisu::e_op) && ((current_exp.get_op_type() == tiramisu::o_access) ||
        ...
        if (insert_access) {
            current_exp.access_vector.insert(current_exp.access_vector.begin(), 0);
        }
