...
void generate_single_coop_cpu() {
    std::string srows = std::to_string(CPU_ROWS);
    std::string scols = std::to_string(COLS);
    function blur("blur_single_coop_cpu");
    std::vector<computation *> comps = algorithm(&blur, srows, scols);
    computation *blur_input = comps[0];
    computation *bx = comps[1];
    computation *by = comps[2];
    ...
    bx->tile(r, c, (int64_t)75, (int64_t)1000, r0, c0, r1, c1);
    by->tile(r, c, (int64_t)75, (int64_t)1000, r0, c0, r1, c1);
    bx->split(r0, 8, r2, r3);
    by->split(r0, 8, r2, r3);
    bx->split(c1, 8, c2, c3);
    by->split(c1, 8, c2, c3);
    ...
    bx->before(*by, computation::root);//r3);

    tiramisu::buffer buff_input("buff_input", {CPU_ROWS+(int64_t)2, COLS+(int64_t)2}, p_float32,
                                tiramisu::a_input, &blur);

    tiramisu::buffer buff_bx("buff_bx", {CPU_ROWS, COLS},
                             p_float32, tiramisu::a_output, &blur);

    tiramisu::buffer buff_by("buff_by", {CPU_ROWS, COLS},
                             p_float32, tiramisu::a_output, &blur);
    ...
    blur.gen_isl_ast();
    blur.gen_halide_stmt();
    blur.gen_halide_obj("/tmp/generated_single_cpu_coop_blur.o");
...
void generate_multi_coop_cpu() {

    std::string srows = std::to_string(CPU_ROWS);
    std::string scols = std::to_string(COLS);
    function blur("blur_multi_coop_cpu");
    std::vector<computation *> comps = algorithm(&blur, srows, scols);
    computation *blur_input = comps[0];
    computation *bx = comps[1];
    computation *by = comps[2];
    ...
    expr border_expr = (blur_input->operator()((int64_t)(CPU_ROWS/(PROCS/2))-(int64_t)2+r,c) +
                        blur_input->operator()((int64_t)(CPU_ROWS/(PROCS/2))-(int64_t)2+r,c+(int64_t)1) +
                        blur_input->operator()((int64_t)(CPU_ROWS/(PROCS/2))-(int64_t)2+r,c+(int64_t)2) +
                        blur_input->operator()((int64_t)(CPU_ROWS/(PROCS/2))-(int64_t)2+r+(int64_t)1,c) +
                        blur_input->operator()((int64_t)(CPU_ROWS/(PROCS/2))-(int64_t)2+r+(int64_t)1,c+(int64_t)1) +
                        blur_input->operator()((int64_t)(CPU_ROWS/(PROCS/2))-(int64_t)2+r+(int64_t)1,c+(int64_t)2) +
                        blur_input->operator()((int64_t)(CPU_ROWS/(PROCS/2))-(int64_t)2+r+(int64_t)2,c) +
                        blur_input->operator()((int64_t)(CPU_ROWS/(PROCS/2))-(int64_t)2+r+(int64_t)2,c+(int64_t)1) +
                        blur_input->operator()((int64_t)(CPU_ROWS/(PROCS/2))-(int64_t)2+r+(int64_t)2,c+(int64_t)2)) / 9.0f;
    computation border("{border[q,r,c]: 0<=q<" + std::to_string((PROCS/2)) + " and 0<=r<2 and 0<=c<" + scols + "}", border_expr, true, p_float32, &blur);

    bx->split(r, CPU_ROWS/(PROCS/2), q1, q2);
    by->split(r, CPU_ROWS/(PROCS/2), q1, q2);

    var c2(p_int64, "c2"), c3(p_int64, "c3");

    bx->tile(q2, c, (int64_t)35, (int64_t)1000, r0, c0, r1, c1);
    by->tile(q2, c, (int64_t)35, (int64_t)1000, r0, c0, r1, c1);
    bx->split(r0, 5, r2, r3);
    by->split(r0, 5, r2, r3);
    bx->split(c1, 8, c2, c3);
    by->split(c1, 8, c2, c3);
    ...
    tiramisu::buffer buff_input("buff_input", {CPU_ROWS/(PROCS/2)+(int64_t)2, COLS+(int64_t)2}, p_float32,
                                tiramisu::a_input, &blur);

    tiramisu::buffer buff_bx("buff_bx", {CPU_ROWS/(PROCS/2), COLS},
                             p_float32, tiramisu::a_output, &blur);

    tiramisu::buffer buff_by("buff_by", {CPU_ROWS/(PROCS/2), COLS},
                             p_float32, tiramisu::a_output, &blur);

    blur_input->set_access("{blur_input[i1, i0]->buff_input[i1,i0]}");
    by->set_access("{by[y, x]->buff_by[y,x]}");
    bx->set_access("{bx[y, x]->buff_bx[y, x]}");
    border.set_access("{border[q,r,c]->buff_by[r+" + std::to_string(CPU_ROWS/(PROCS/2)) + "-2,c]}");
    blur.lift_dist_comps();
    blur.set_arguments({&buff_input, &buff_bx, &buff_by});
    blur.gen_time_space_domain();
    blur.gen_isl_ast();
    blur.gen_halide_stmt();
    blur.gen_halide_obj("/tmp/generated_multi_cpu_coop_blur.o");
...
void generate_single_coop_gpu() {

    std::string srows = std::to_string(GPU_ROWS);
    std::string scols = std::to_string(COLS);
    std::string sresident = std::to_string(RESIDENT);
    function blur("blur_single_coop_gpu");
    var r(p_int64, "r"), r0(p_int64, "r0"), r1(p_int64, "r1");
    var c(p_int64, "c"), c0(p_int64, "c0"), c1(p_int64, "c1");
    var z(p_int64, "z");
    ...
    xfer ghost = computation::create_xfer("{ghost[r,z,c]: 0<=r<(" + std::to_string(GPU_ROWS/RESIDENT) + ") and 0<=z<4 and 0<=c<2+" + scols +
                                          "}", h2d_cuda_async_kernel_stream, blur_input->operator()(/*(r+(int64_t)1)*(GPU_ROWS/RESIDENT)+z*/r*RESIDENT+(int64_t)RESIDENT+z-(int64_t)2,c), &blur);
    //  ghost.os->split(r, 2, r0, r1);
    ghost.os->collapse_many({collapser(2, (int64_t)0, COLS+(int64_t)2)});
    ...
    // buffers
    // add on +2 to the rows in case we are actually needing the next rank's rows (we will need to do the blur on those separately)
    buffer b_blur_input("b_blur_input", {GPU_ROWS+(int64_t)2, COLS+(int64_t)2}, p_float32, a_input, &blur);
    buffer b_blur_input_gpu("b_blur_input_gpu", {RESIDENT+(int64_t)2, COLS+(int64_t)2}, p_float32, a_temporary_gpu, &blur);
    // the last two rows are junk rows, but we need to make sure we don't give ourselves an
    // out of bound access
    buffer b_bx_gpu("b_bx_gpu", {RESIDENT+(int64_t)2, COLS}, p_float32, a_temporary_gpu, &blur);
    ...
    buffer b_border("b_border", {(int64_t)4, COLS}, p_float32, a_temporary_gpu, &blur);
    buffer b_by_gpu("b_by_gpu", {RESIDENT, COLS}, p_float32, a_temporary_gpu, &blur);
    buffer b_by("b_by", {GPU_ROWS, COLS}, p_float32, a_output, &blur);
    buffer b_h2d_wait("b_h2d_wait", {GPU_ROWS, 1}, p_wait_ptr, a_temporary, &blur);
    buffer b_d2h_wait("b_d2h_wait", {GPU_ROWS, 1}, p_wait_ptr, a_temporary, &blur);
    buffer b_d2h_wait_for_by("b_d2h_wait_for_by", {GPU_ROWS, 1}, p_wait_ptr, a_temporary, &blur);
    buffer b_ghost_wait("b_ghost_wait", {(int64_t)4*GPU_ROWS/RESIDENT, 1}, p_wait_ptr, a_temporary, &blur);
    // buffers that are required for code gen, but we don't directly use
    ...
    buffer buff_null("buff_null", {GPU_ROWS,2}, p_wait_ptr, a_temporary, &blur);
    ...
    blur.gen_isl_ast();
    blur.gen_halide_stmt();
    blur.gen_halide_obj("/tmp/generated_single_gpu_coop_blur.o");
    blur.dump_halide_stmt();
    compile_kernels_to_obj();
}
...
void generate_multi_gpu() {
    ...
    buffer b_border("b_border", {(int64_t)4, COLS}, p_float32, a_temporary_gpu, &blur);
    buffer b_by_gpu("b_by_gpu", {RESIDENT, COLS}, p_float32, a_temporary_gpu, &blur);
    buffer b_by("b_by", {ROWS/PROCS, COLS}, p_float32, a_output, &blur);
    buffer b_h2d_wait("b_h2d_wait", {ROWS/PROCS, 1}, p_wait_ptr, a_temporary, &blur);
    buffer b_d2h_wait("b_d2h_wait", {ROWS/PROCS, 1}, p_wait_ptr, a_input, &blur);
    buffer b_d2h_wait_for_by("b_d2h_wait_for_by", {ROWS/PROCS, 1}, p_wait_ptr, a_temporary, &blur);
    buffer b_ghost_wait("b_ghost_wait", {(int64_t)4*ROWS/PROCS/RESIDENT, 1}, p_wait_ptr, a_temporary, &blur);
    // buffers that are required for code gen, but we don't directly use
    buffer buff_bx_literals("buff_bx_literals", {ROWS, (int64_t)1}, p_int64, a_temporary_gpu, &blur);
    buffer buff_border_literals("buff_border_literals", {ROWS, (int64_t)1}, p_int64, a_temporary_gpu, &blur);
    buffer buff_by_literals("buff_by_literals", {ROWS, (int64_t)1}, p_int64, a_temporary_gpu, &blur);
    buffer buff_null("buff_null", {ROWS/PROCS,2}, p_wait_ptr, a_temporary, &blur);
...
void generate_multi_coop_gpu() {
    std::string srows = std::to_string(GPU_ROWS);
    std::string scols = std::to_string(COLS);
    std::string sresident = std::to_string(RESIDENT);
    function blur("blur_multi_coop_gpu");
    blur.rank_offset = -4;
    var r(p_int64, "r"), r0(p_int64, "r0"), r1(p_int64, "r1");
    var c(p_int64, "c"), c0(p_int64, "c0"), c1(p_int64, "c1");
    var z(p_int64, "z");
    ...
    bx->split(r, GPU_ROWS/(PROCS/2), q, rq);
    by->split(r, GPU_ROWS/(PROCS/2), q, rq);
    ...
    generator::update_producer_expr_name(bx, "blur_input", "h2d", false);
    // split up the h2d transfer based on how much we can transfer at a time
    h2d.os->split(r, GPU_ROWS/(PROCS/2), q, rq);
    h2d_wait.split(r, GPU_ROWS/(PROCS/2), q, rq);
    h2d.os->split(rq, RESIDENT, r2, r1);
    h2d_wait.split(rq, RESIDENT, r2, r1);
    // collapse the h2d so it sends the whole row as a chunk
    h2d.os->collapse_many({collapser(3, (int64_t)0, COLS+(int64_t)2)});
    ...
    xfer d2h = computation::create_xfer("{d2h[r,c]: 0<=r<" + srows + " and 0<=c<" + scols +
                                        "}", d2h_cuda_async, by->operator()(r,0), &blur);
    tiramisu::wait d2h_wait("{d2h_wait[r,c]: 0<=r<" + srows + " and 0<=c<1}", d2h.os->operator()(r-RESIDENT,0), h2d_cuda_async, true, &blur);
    d2h.os->split(r, GPU_ROWS/(PROCS/2), q, rq);
    d2h_wait.split(r, GPU_ROWS/(PROCS/2), q, rq);
    d2h_wait_for_by.split(r, GPU_ROWS/(PROCS/2), q, rq);
    ...
    xfer ghost = computation::create_xfer("{ghost[r,z,c]: 0<=r<(" + std::to_string(GPU_ROWS/RESIDENT) + ") and 0<=z<4 and 0<=c<2+" + scols +
                                          "}", h2d_cuda_async_kernel_stream, blur_input->operator()(r*RESIDENT+(int64_t)RESIDENT+z-(int64_t)2,c), &blur);
    ghost.os->split(r, GPU_ROWS/RESIDENT/(PROCS/2), q, rq);
    ghost.os->collapse_many({collapser(3, (int64_t)0, COLS+(int64_t)2)});
    ...
    expr border_expr = (ghost.os->operator()(0, z, c) + ghost.os->operator()(0, z, c+(int64_t)1) + ghost.os->operator()(0, z, c+(int64_t)2) +
                        ...
                        ghost.os->operator()(0, z+(int64_t)2, c) + ghost.os->operator()(0, z+(int64_t)2, c+(int64_t)1) + ghost.os->operator()(0, z+(int64_t)2, c+(int64_t)2)) / 9.0f;
    computation border("{border[r,z,c]: 0<=r<" + std::to_string(GPU_ROWS/RESIDENT) + " and 0<=z<2 and 0<=c<" + scols + "}", border_expr, true, p_float32, &blur);
    border.split(r, GPU_ROWS/RESIDENT/(PROCS/2), q, rq);
    border.split(c, BLOCK_SIZE, c0, c1);

    xfer d2h_border = computation::create_xfer("{d2h_border[r,z,c]: 0<=z<2 and 0<=r<" + std::to_string(GPU_ROWS/RESIDENT) + " and 0<=c<" + scols + "}", d2h_cuda_async_kernel_stream, border(r,z,c), &blur);
    d2h_border.os->split(r, GPU_ROWS/RESIDENT/(PROCS/2), q, rq);
    d2h_border.os->collapse_many({collapser(3, (int64_t)0, COLS)});
    tiramisu::wait d2h_border_wait("{d2h_border_wait[r,z,c]: 0<=z<2 and 0<=r<" + std::to_string(GPU_ROWS/RESIDENT) + " and 0<=c<1}", d2h_border.os->operator()(r,z,0), stream1, true, &blur);
    d2h_border_wait.split(r, GPU_ROWS/(PROCS/2), q, rq);
    d2h_border_wait.set_schedule_this_comp(false);
    ...
    // buffers
    // add on +2 to the rows in case we are actually needing the next rank's rows (we will need to do the blur on those separately)
    buffer b_blur_input("b_blur_input", {GPU_ROWS/(PROCS/2)+(int64_t)2, COLS+(int64_t)2}, p_float32, a_input, &blur);
    buffer b_blur_input_gpu("b_blur_input_gpu", {RESIDENT+(int64_t)2, COLS+(int64_t)2}, p_float32, a_temporary_gpu, &blur);
    // the last two rows are junk rows, but we need to make sure we don't give ourselves an
    // out of bound access
    buffer b_bx_gpu("b_bx_gpu", {RESIDENT+(int64_t)2, COLS}, p_float32, a_temporary_gpu, &blur);
    ...
    buffer b_border("b_border", {(int64_t)4, COLS}, p_float32, a_temporary_gpu, &blur);
    buffer b_by_gpu("b_by_gpu", {RESIDENT, COLS}, p_float32, a_temporary_gpu, &blur);
    buffer b_by("b_by", {GPU_ROWS/(PROCS/2), COLS}, p_float32, a_output, &blur);
    buffer b_h2d_wait("b_h2d_wait", {GPU_ROWS/(PROCS/2), 1}, p_wait_ptr, a_temporary, &blur);
    buffer b_d2h_wait("b_d2h_wait", {GPU_ROWS/(PROCS/2), 1}, p_wait_ptr, a_input, &blur);
    buffer b_d2h_wait_for_by("b_d2h_wait_for_by", {GPU_ROWS/(PROCS/2), 1}, p_wait_ptr, a_temporary, &blur);
    buffer b_ghost_wait("b_ghost_wait", {(int64_t)4*GPU_ROWS/(PROCS/2)/RESIDENT, 1}, p_wait_ptr, a_temporary, &blur);
    // buffers that are required for code gen, but we don't directly use
    buffer buff_bx_literals("buff_bx_literals", {GPU_ROWS, (int64_t)1}, p_int64, a_temporary_gpu, &blur);
    buffer buff_border_literals("buff_border_literals", {GPU_ROWS, (int64_t)1}, p_int64, a_temporary_gpu, &blur);
    buffer buff_by_literals("buff_by_literals", {GPU_ROWS, (int64_t)1}, p_int64, a_temporary_gpu, &blur);
    buffer buff_null("buff_null", {GPU_ROWS/(PROCS/2),2}, p_wait_ptr, a_temporary, &blur);
    ...
    blur.gen_isl_ast();
    blur.gen_halide_stmt();
    blur.gen_halide_obj("/tmp/generated_multi_gpu_coop_blur.o");
    blur.dump_halide_stmt();
    compile_kernels_to_obj();
}

void generate_multi_cpu_comm_border() {

    std::string srows = std::to_string(ROWS);
    std::string scols = std::to_string(COLS);
    function blur("blur_multi_cpu_comm_border");
    std::vector<computation *> comps = algorithm(&blur, srows, scols);
    computation *blur_input = comps[0];
    computation *bx = comps[1];
    computation *by = comps[2];
    ...
    blur.gen_isl_ast();
    blur.gen_halide_stmt();
    blur.gen_halide_obj("/tmp/generated_multi_cpu_border_comm_blur.o");
...
#ifdef BORDER_COMM
#ifdef CPU
    generate_multi_cpu_comm_border();
#else
    assert(false);
#endif
#else
#ifdef GPU
    #ifdef DIST
    std::cerr << "Generating multi GPU" << std::endl;
    generate_multi_gpu();
#else
    generate_single_gpu();
#endif
#elif defined(COOP)
    assert(PROCS >= 2 && "Need at least 1 proc for CPU and GPU on cooperative");
    if (PROCS == 2) {
      generate_single_coop_cpu();
      generate_single_coop_gpu();
    } else {
      generate_multi_coop_cpu();
      generate_multi_coop_gpu();
    }
#else
#ifdef DIST
