...
protected:
    ...
    /**
      * Is this statement a library call? If so, treat this differently.
      * This operation may not have a RHS or LHS accessor.
      */
...
protected:
    ...
    /**
      ...
      *
      * This is different from an operation's expression being a function call. If an operation's expression
      * is a function call, that means the function call is used in the RHS of the assignment. If _is_function_call is
      * true, the entire computation itself is a function call, so there is no explicit assignment (the assignment may
    ...
    /**
      ...
      * the LHS and RHS accessors (if they are used).
      */
    std::vector<tiramisu::expr> library_call_args;

    /**
      * If this operation is a library call, this is the name of the library call.
      */
    std::string library_call_name;

    /**
      * If this operation is a library call that wants to make use of LHS and RHS accessors, these maps say which position
      * in the library call argument list the LHS or RHS belongs.
      */
