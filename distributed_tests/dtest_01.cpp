...
int main(int argc, char **argv) {
    ...
    function dtest01("dtest01_func");

    // Layer 1

    computation x_input("{x_input[i,j]: 0<=i<160 and 0<=j<160}", expr(), false, p_float32, &dtest01);
    expr e = expr(x_input(var("i"), var("j")) + expr((float_t) 4));
    computation S("{S[i,j]: 0<=i<160 and 0<=j<160}", e, true, p_float32, &dtest01);
    ...
    // Create buffers
    buffer x_input_buff("x_input_buff", {160, 160}, p_float32, a_input, &dtest01);
    buffer SA_buff("SA_buff", {160, 160}, p_float32, a_output, &dtest01);
    buffer SB_buff("SB_buff", {80, 160}, p_float32, a_temporary, &dtest01);
    buffer nB_recv_buff("nB_recv_buff", {80, 160}, p_float32, a_temporary, &dtest01);
    ...
    // Code generation
    dtest01.gen_time_space_domain();
    dtest01.set_arguments({&x_input_buff, &SA_buff});
    dtest01.lift_ops_to_library_calls();
    dtest01.gen_isl_ast();
    dtest01.gen_halide_stmt();
    dtest01.dump_halide_stmt();
//    dtest01.gen_halide_obj("TBD");
