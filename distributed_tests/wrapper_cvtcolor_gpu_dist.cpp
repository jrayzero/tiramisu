...
#include <fstream>
#include <mpi.h>
#include "wrapper_cvtcolor_gpu_dist.h"
#include "Halide.h"
...
int main() {
    ...
    for (int i=0; i<20; i++) {
        ...
        /*	if (i == 0) {
          std::string output_fn = "/data/scratch/jray/tiramisu/build/cvtcolor_gpu_dist_rank_" + std::to_string(rank) + ".txt";
          std::ofstream myfile;
    ...
    if (rank == 0) {
        print_time("performance_GPU.csv", "cvtcolor_gpu_dist", {"Tiramisu_dist"}, {median(duration_vector)});
    }
