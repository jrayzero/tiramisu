...
int main(int, char **)
{
  ...
  Halide::Buffer<uint8_t> *image = nullptr;
    ...
    Halide::Buffer<uint64_t> buff_input_temp(COLS, ROWS);
    Halide::Buffer<uint64_t> buff_bx_inter(COLS-6, ROWS);
    Halide::Buffer<uint64_t> buff_by_inter(COLS-8, ROWS);
    ...
    *image = Halide::Tools::load_image("./images/rgb.png");
    std::cerr << "Loaded" << std::endl;
        ...
        buf = (uint64_t*)malloc(sizeof(uint64_t) * ROWS * COLS * CHANNELS);
        uint64_t v = 0;
        size_t next = 0;
        ...
        *image = Halide::Buffer<uint64_t>(buf, {COLS, ROWS, CHANNELS});
#endif
    }
  ...
  std::cerr << "loaded reference" << std::endl;
    ...
    Halide::Buffer<uint8_t> output_buf(rank == 0 ? image->extent(0) - 8 : 0,
				     rank == 0 ? image->extent(1) - 8 : 0, rank == 0 ? image->channels() : 0);
